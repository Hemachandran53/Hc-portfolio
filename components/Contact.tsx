...
import { CONTACT_INFO, MailIcon, PhoneIcon, LocationIcon, LinkedInIcon, GitHubIcon } from '../constants';
...
const ContactInfoItem: React.FC<{ icon: React.FC<any>, text: string, href?: string }> = ({ icon: Icon, text, href }) => (
...
const Contact: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    // For a real app, you would send this data to a backend or email service.
    // For this example, we'll construct a mailto link or show a success message.
    ...
    // Option 2: Simulate success (if no mailto needed)
    // setStatusMessage('Message sent successfully! (Not really, this is a demo)');
    // setFormData({ name: '', email: '', message: '' });
    // setTimeout(() => setStatusMessage(''), 5000);
  };

  return (
    <section id="contact" className="py-16 md:py-24 bg-lightBgAccent dark:bg-darkBgAccent">
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {statusMessage && (
                <p className="text-sm text-center text-green-600 dark:text-green-400">{statusMessage}</p>
              )}
            </form>
          </div>
        </div>
